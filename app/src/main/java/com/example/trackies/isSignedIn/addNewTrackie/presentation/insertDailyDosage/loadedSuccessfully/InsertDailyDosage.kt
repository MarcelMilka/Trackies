package com.example.trackies.isSignedIn.addNewTrackie.presentation.insertDailyDosage.loadedSuccessfully
...
@Composable fun insertDailyDosage(viewModel: AddNewTrackieViewModel) {
...
//  user's data
    ...
    var targetHeightOfTheColumn by remember {
        mutableIntStateOf(
            InsertDailyDosageFixedHeightValues.displayUnactivatedComponent
        )
    }
    ...
    var hint by remember {
        mutableStateOf(
            DailyDosageHints.insertDailyDosage
        )
    }
...
//  Control whether this segment is active
    ...
    CoroutineScope(Dispatchers.Default).launch {
        ...
        viewModel.statesOfSegments.collect { statesOfSegments ->
            thisSegmentIsActive = statesOfSegments.insertTotalDoseIsActive

            when(thisSegmentIsActive) {
                true -> {
                    when(measuringUnit) {
                        null -> viewModel.dailyDoseDisplayMeasuringUnitsToChoose()
                        else -> viewModel.dailyDoseDisplayTextField()
                    }
                }
                false -> {
                    when(measuringUnit) {
                        null -> viewModel.dailyDoseDisplayCollapsed()
                        else -> viewModel.dailyDoseDisplayInsertedValue()
                    }
                }
            }
        }
...
//  Update values
    CoroutineScope(Dispatchers.Default).launch {
        ...
        viewModel.insertDailyDosageViewState.collect {
...
//                      Collapse this segment
                        true -> {
                            viewModel.deactivateSegment(
                                segmentToDeactivate = AddNewTrackieSegments.DailyDosage
                            )

                            viewModel.updateMeasuringUnitAndDose(
                                totalDose = totalDailyDose,
                                measuringUnit = measuringUnit.toString()
                            )

                            viewModel.dailyDoseDisplayInsertedValue()
                        }
...
//                      Expand this segment
                        false -> {
                            ...
                            viewModel.activateSegment(
                                segmentToActivate = AddNewTrackieSegments.DailyDosage
                            )

                            if (measuringUnit == null) {
                                viewModel.dailyDoseDisplayMeasuringUnitsToChoose()
                            }

                            else {
                                viewModel.dailyDoseDisplayTextField()
                            }
                        }
                    }
                },
                                            ...
                                            Row(
                                                ...
                                                content = {
                                                    ...
                                                    mediumRadioTextButton(text = MeasuringUnits.ml, isSelected = mlIsChosen) {
                                                        ...
                                                        viewModel.dailyDoseInsertMeasuringUnit(measuringUnit = EnumMeasuringUnits.ml)
                                                        viewModel.dailyDoseDisplayTextField()
                                                    }
                                                    ...
                                                    mediumRadioTextButton(text = MeasuringUnits.g, isSelected = gIsChosen) {
                                                        ...
                                                        viewModel.dailyDoseInsertMeasuringUnit(measuringUnit = EnumMeasuringUnits.g)
                                                        viewModel.dailyDoseDisplayTextField()
                                                    }
                                                    ...
                                                    mediumRadioTextButton(text = MeasuringUnits.pcs, isSelected = pcsIsChosen) {
                                                        ...
                                                        viewModel.dailyDoseInsertMeasuringUnit(measuringUnit = EnumMeasuringUnits.pcs)
                                                        viewModel.dailyDoseDisplayTextField()
                                                    }
                                                }
                                            )
...
//                                            Divider()
                                            ...
                                            TextField(
                                                ...
                                                onValueChange = {
                                                    ...
                                                    viewModel.dailyDoseInsertTotalDose(totalDose = newValue.toIntOrNull() ?: 0 )
                                                },
